...
type UserProfileData = {
  ...
  birthDate?: Date; // User's birth date (optional)
...
export const register = createAsyncThunk(
  ...
  async (data: NewUser, { rejectWithValue }) => {
    try {
      ...
      const userDataWithAvatar = { ...data, avatar: defaultAvatarUrl };
...
export const updateUser = createAsyncThunk(
  'users/update',
  async ({ userId, updateData }: { userId: string; updateData: UserProfileData }, { rejectWithValue }) => {
    try {
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateUser.fulfilled, (state, action: PayloadAction<UserProfileData>) => {
        state.status = 'idle';
        state.userProfileData = action.payload;
      })
      .addCase(updateUser.rejected, (state, action) => {
        state.status = 'failed';
        state.error = action.payload ? (action.payload as ErrorResponse).message : action.error.message || 'Update user failed';
      });
  },
});
